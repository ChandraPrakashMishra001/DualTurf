...
function LoginForm() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      let userMsg = msg || 'Google sign-in failed. Please try again.';
      if (code === 'auth/operation-not-allowed' || msg.includes('operation-not-allowed')) {
        userMsg = '⚠️ Google Provider is disabled. Enable Google in Firebase Console → Authentication → Sign-in method.';
      } else if (code === 'auth/unauthorized-domain' || msg.includes('unauthorized-domain')) {
        userMsg = '⚠️ Domain unauthorized! Add dualturf.in and www.dualturf.in in Firebase Console → Authentication → Settings → Authorized domains.';
      } else if (code === 'auth/popup-blocked' || msg.includes('popup-blocked')) {
        userMsg = '⚠️ Sign-in popup was blocked by your browser. Please allow popups or use Email & Password.';
      }
      setError(userMsg);
    } finally {
      setSubmitting(false);
    ...
    <div className={styles.container}>
      <div className={styles.formWrapper}>
        ...
        <button
          ...
          style={{
            width: '100%',
            backgroundColor: submitting ? '#e0e0e0' : '#ffffff',
            color: '#000000',
            border: 'none',
